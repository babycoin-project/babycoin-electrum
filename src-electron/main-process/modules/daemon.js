    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let evolutiond_path = path.join(__evolution_bin, "babycoind.exe")
                ...
                let evolutiond_path = path.join(__evolution_bin, "babycoind")
                let evolutiond_version_cmd = `"${evolutiond_path}" --version`
                if (!fs.existsSync(evolutiond_path))
                    resolve(false)
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "babycoind.log"))
                //args.push("--add-peer", "45.77.68.151:13310")
            } else {
                args.push("--log-file", path.join(options.app.data_dir, "logs", "babycoind.log"))
            }
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__evolution_bin, "babycoind.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__evolution_bin, "babycoind"), args, {
                    detached: true
                })
            }
